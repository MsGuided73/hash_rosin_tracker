...
// otherwise fall back to the public-dir path served by Vite at the site root.
function MICRON_ASSET(id, fallback) {
  return (window.__resources && window.__resources[id]) || fallback;
}

export const MicronTokens = {
  // spacing (4px grid)
...
export function withStage(theme, stageId) {
  const base = MicronTokens[theme];
