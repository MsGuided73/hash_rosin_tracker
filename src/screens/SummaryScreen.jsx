...
import { Btn, BandChip, formatG } from '../components/primitives.jsx';
import { TopBar, BottomBar } from '../components/Chrome.jsx';
import { SectionLabel } from './FreezeDryScreen.jsx';
import { PressCard } from './PressScreen.jsx';

// Summary screen — yields, breakdowns, press runs
export function SummaryScreen({ batch, onBack, onEdit, unit, theme }) {
  const t = withStage(theme, 'summary');
  const bands = MicronBands;
        ...
        <div style={{ padding: '18px 16px 0' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {batch.mergedGrades.map(m => {
              const dry = GradeUnits.mergeDry(batch, m);
...
export function FlowBar({ segments, unit, theme }) {
