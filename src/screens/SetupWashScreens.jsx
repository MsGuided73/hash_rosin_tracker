...
import { Icon, Btn, Stepper, ScaleFace, NumberPad, BandChip, Input, formatG } from '../components/primitives.jsx';
import { PhotoNotes, ComboField, DateField } from '../components/PhotoNotes.jsx';
import { TopBar, BottomBar } from '../components/Chrome.jsx';
const { useState: useStateS } = React;

export function SetupScreen({ draft, setDraft, onContinue, onCancel, theme }) {
    ...
    <div style={{ background: t.bgGradient || t.bg, height: '100%', color: t.text, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '0 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        ...
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginTop: -8 }}>
          {MicronSeed.strains.slice(0,5).map(s => (
...
export function WashScreen({ batch, setBatch, onContinue, onBack, unit, theme }) {
  const t = withStage(theme, 'wash');
  const [activeBag, setActiveBag] = useStateS(null);

  const enteredCount = MicronBands.filter(b => batch.bags[b.id].wet != null).length;
...
// Shared chrome (TopBar / BottomBar) now lives in ../components/Chrome.jsx.
