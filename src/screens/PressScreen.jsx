...
import { Icon, Btn, Stepper, NumberPad, Sheet, UnitChip, formatG } from '../components/primitives.jsx';
import { PhotoNotes } from '../components/PhotoNotes.jsx';
import { TopBar, BottomBar } from '../components/Chrome.jsx';
import { SectionLabel } from './FreezeDryScreen.jsx';

// Press builder screen — multi-run, flexible grade combining
const { useState: useStatePR } = React;

export function PressScreen({ batch, setBatch, onContinue, onBack, unit, theme }) {
  const t = withStage(theme, 'press');
...
export function YieldByMicronSummary({ batch, unit, theme }) {
...
export function PressCard({ run, index, unit, theme, batch }) {
  const t = (window.__stage && withStage) ? withStage(theme, window.__stage) : MicronTokens[theme];
    ...
    <div style={{
      ...
      <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap', marginTop: 10 }}>
        {run.grades.map(g => {
          ...
          const u = batch ? GradeUnits.resolveUnit(batch, g) : { kind: 'band', label: g + 'µ', bandIds: [g] };
