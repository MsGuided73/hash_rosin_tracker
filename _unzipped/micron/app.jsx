...
function MicronApp() {
  ...
  const [batches, setBatches] = useStateA(() => {
    ...
    return s ? JSON.parse(s) : window.MicronSeed.batches();
  });
  ...
  const createNewBatch = () => {
    ...
    const draft = {
      ...
      stage: 'setup',
      notes: '',
      bags: Object.fromEntries(window.MicronBands.map(b => [b.id, { wet: null, dry: null, color: null, melt: 0, texture: '', notes: '' }])),
      mergedGrades: [],
      presses: [],
    };
  ...
  return (
    <>
      ...
      <TweaksPanel
        ...
        onReset={() => {
          setBatches(window.MicronSeed.batches());
          setRoute({ name: 'home' });
          setTweaksOpen(false);
        }}/>
    </>
...
Object.assign(window, { MicronApp });
