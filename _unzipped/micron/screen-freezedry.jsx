...
function FreezeDryScreen({ batch, setBatch, onContinue, onBack, unit, theme }) {
  const t = window.withStage(theme, 'freezedry');
  const GU = window.GradeUnits;

  const [activeId, setActiveId] = useStateFD(window.MicronBands[0].id);
  ...
  const enteredCount = window.MicronBands.filter(b => batch.bags[b.id].dry != null).length;
      ...
      <div style={{
        ...
        {isMerge ? (
          <div style={{ marginTop: 18 }}>
            ...
            <button onClick={() => { setBatch(b => GU.removeMerge(b, merge.id)); setActiveId(window.MicronBands[0].id); }}
...
function CombineSheet({ open, onClose, batch, theme, unit, onCreate }) {
  const t = window.withStage(theme, 'freezedry');
  const GU = window.GradeUnits;
  ...
  const available = window.MicronBands.filter(b => !inMerge.has(b.id));
...
Object.assign(window, { FreezeDryScreen, CombineSheet, SectionLabel });
