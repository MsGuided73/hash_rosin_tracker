...
// ─────────── Utilities ───────────
function useTheme(mode = 'dark') {
  const base = window.MicronTokens[mode];
  const stageId = window.__stage;
  if (!stageId || !window.MicronStages) return base;
...
// ─────────── Button ───────────
function Btn({ kind = 'primary', children, onClick, disabled, style, icon, fullWidth }) {
...
function Stepper({ current, onJump }) {
  ...
  const stageColor = (id) => {
    const stage = window.MicronStages && window.MicronStages[id];
    ...
    <div style={{
      ...
      {steps.map((s, i) => {
        ...
        const dotInk = (active || done)
          ? (window.MicronStages[s.id] && window.MicronStages[s.id][t.name] && window.MicronStages[s.id][t.name].accentInk) || t.accentInk
          : t.textDim;
        return (
...
function ScaleFace({ value, unit = 'g', label, onTap, size = 'lg', bandColor, placeholder = '— —' }) {
  const t = useTheme(window.__theme || 'dark');
  const big = size === 'lg';
...
// ─────────── Number pad (sheet) ───────────
function NumberPad({ open, onClose, initial = '', onSubmit, title, unit = 'g', helper }) {
  const t = useTheme(window.__theme || 'dark');
...
function MeltRating({ value = 0, onChange, max = 6 }) {
  const t = useTheme(window.__theme || 'dark');
  return (
...
// ─────────── Input ───────────
function Input({ value, onChange, placeholder, label, mono, multi, style }) {
  const t = useTheme(window.__theme || 'dark');
...
function BandChip({ bandId, size = 'md' }) {
  const t = useTheme(window.__theme || 'dark');
  const color = t.bands[bandId];
  const band = window.MicronBands.find(b => b.id === bandId);
...
function UnitChip({ batch, id, size = 'md' }) {
  const t = useTheme(window.__theme || 'dark');
  const u = window.GradeUnits.resolveUnit(batch, id);
...
Object.assign(window, {
  Icon, Btn, Stepper, ScaleFace, NumberPad, Sheet,
  ColorSwatches, MeltRating, Input, BandChip, UnitChip, Segmented,
  useTheme, formatG, injectGlobalCSS,
});
